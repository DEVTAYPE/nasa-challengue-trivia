    ...
    return (
      <div className="results-panel">
        <div className="welcome">
          <h3>🌍 Bienvenido</h3>
          <p>
            Haz click en cualquier lugar del mapa para obtener recomendaciones
            de cultivos.
          </p>
      ...
      <div className="location-info">
        ...
        {locationInfo ? (
          <>
            <p>
              <strong>Lugar:</strong>{" "}
              {locationInfo.display_name || "Ubicación desconocida"}
            </p>
            <p>
              <strong>Coordenadas:</strong> {selectedLocation?.lat?.toFixed(4)},{" "}
      ...
      <div className="recommendations">
        ...
        {top_recommendations && top_recommendations.length > 0 ? (
          <div className="crop-list">
            {top_recommendations.map((crop: any, index: number) => (
              <div key={index} className="crop-item">
                ...
                <div className="crop-details">
                  <p>
                    <strong>Mejor época:</strong> {crop.best_planting_window}
                  </p>
                  <p>
                    <strong>Confianza:</strong> {crop.confidence_level}
                  </p>
                  {crop.climate_shift_detected && (
                    <p className="climate-warning">
                      ⚠️ Cambio climático detectado
                    </p>
                  )}
                </div>
                  ...
                  <div className="adaptive-factors">
                    ...
                    <div className="factors-grid">
                      <div className="factor-item">
                        <span>NDVI:</span>
                        <span>
                          {crop.adaptive_factors.ndvi_score?.toFixed(2)}
                        </span>
                      </div>
                      <div className="factor-item">
                        <span>Temperatura:</span>
                        <span>
                          {crop.adaptive_factors.temperature_score?.toFixed(2)}
                        </span>
                      </div>
                      <div className="factor-item">
                        <span>Humedad:</span>
                        <span>
                          {crop.adaptive_factors.soil_moisture_score?.toFixed(
                            2
                          )}
                        </span>
                      </div>
                      <div className="factor-item">
                        <span>Pendiente:</span>
                        <span>
                          {crop.adaptive_factors.slope_score?.toFixed(2)}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="frost-assessment">
                    ...
                    <div className="frost-details">
                      <p>
                        ...
                        {crop.frost_risk_assessment.frost_tolerance}
                      </p>
                      <p>
                        <strong>Impacto:</strong>{" "}
                        {(
                          crop.frost_risk_assessment.frost_impact_score * 100
                        ).toFixed(1)}
                        %
                      </p>
                      <p>
                        <strong>Riesgo Estacional:</strong>{" "}
                        {crop.frost_risk_assessment.seasonal_frost_risk}
                      </p>
                    </div>
                  </div>
                )}
